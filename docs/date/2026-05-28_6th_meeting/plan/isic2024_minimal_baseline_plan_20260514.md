...
baseline의 목적은 다음 질문에 답하는 것

```text
1. Imbalance data 가 학습에 문제 없도록 전처리되었는가? 
2. tabular metadata만으로 어느 정도 성능이 나오는가?
3. lesion image만으로 어느 정도 성능이 나오는가?
4. image + tabular multimodal baseline은 어느 정도 성능이 나오는가?
```

## 2. :speaker:현재 baseline 범위

현재 구현 기준의 baseline 범위는 아래와 같다.

| No | 구분 | 현재 상태 |
|---|---|---|
| 1 | split | patient-level Triple Stratified Nested CV, 5 outer x 4 inner |
| 2 | tabular baseline | `xgboost`, `catboost`, `lightgbm`, `ft_transformer`, `ft_transformer_external`, `logistic_regression`, `svm`, `mlp` |
| 3 | image baseline | `resnet50`, `efficientnetv2_s`, `convnextv2_tiny`, `eva02_s`, `vit_b`, `edgenext_s` |
| 4 | multimodal | 아직 훈련 미구현(early, joint, late fusion 방식, MONET(ISIC 2024 공식 sample solution)) |
...
참조 문서: [isic2024_strict_input_export.md](../../../eda/isic2024_strict_input_export.md)
...
### 4.3 Multimodal scaffold와 roadmap
...
후속 구현 우선순위는 다음과 같다. 이 로드맵은 메인 프로젝트 정체성이 아니라, tabular/image baseline 안정화 이후의 multimodal baseline 및 ablation 순서로 해석한다.
