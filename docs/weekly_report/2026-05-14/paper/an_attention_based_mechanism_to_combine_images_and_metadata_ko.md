...
이 파일은 원 논문의 흐름을 따라 한국어로 옮긴 번역본이다. 원문에 실린 그림은 원 논문 PDF에서 추출한 그림 파일만 사용했고, 그림 캡션은 원문 의미에 맞추어 번역했다. 연구 프로젝트와 연결되는 해석은 본문 번역과 섞지 않고 `ISIC2024 연구 코멘트 (번역 아님)` 블록으로만 분리했다.
...
> MetaBlock은 `image + ordinary tabular metadata` 방향과 직접 맞닿아 있다. ISIC2024에서는 `iddx_full` 같은 진단 파생 필드가 아니라 나이, 성별, 병변 위치, 3D-TBP 계측값처럼 추론 시점에 사용 가능한 ordinary metadata만 gate 입력으로 제한해야 한다.
...
> **ISIC2024 연구 코멘트 (번역 아님)**
> 이 논문의 비교 축은 우리 프로젝트의 fusion ablation 설계에 그대로 참고할 수 있다. 최소 비교군은 image-only, tabular-only, 단순 concatenation, metadata-gated feature modulation이어야 하며, 모든 비교는 같은 patient-level fold와 같은 pAUC above TPR 0.80 구현을 공유해야 한다.
...
이 절은 원문과의 대조를 돕기 위한 안내이다. 본문 번역에는 표의 핵심 성능 수치와 해석을 반영했고, 상세한 표 layout은 원문 PDF의 다음 항목과 대응된다.
