...
9. ISIC2024 multimodal 연구에 주는 시사점
...
주의: IEEE 원문 figure는 직접 삽입하지 않고, 논문 이해를 위한 자체 schematic으로 대체한다.

**자체 Figure. MetaBlock의 metadata-conditioned feature gating**
...
flowchart LR
    ...
    M["Metadata vector x_meta"]

    subgraph MB["MetaBlock 범위"]
        direction LR
        F["Linear f_b(x_meta)\nscale modifier"]
        G["Linear g_b(x_meta)\nshift modifier"]
        H["tanh(f_b(x_meta) * x_img) + g_b(x_meta)"]
        I["sigmoid gate\n0~1 feature weights"]
        J["Metadata-aware\nvisual feature"]
    end
    ...
    I --> J
    J --> K["Classifier"]
    K --> L["Class probability"]
```

핵심 해석:

- metadata encoder는 복잡한 tabular backbone이 아니라 metadata vector에서 scale/shift modifier를 만드는 linear layer에 가깝다.
