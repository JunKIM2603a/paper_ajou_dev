...
| 계층 | 책임 | 현재 파일 |
|---|---|---|
| execution core (`.py`) | data loading, split, model build, metric 계산, trainer | `src/isic2024_benchmark/data.py`, `tabular_data.py`, `models.py`, `metrics.py`, `trainer.py` |
| execution entrypoint (`.py`) | 실제 benchmark 실행, multi-GPU scheduling, artifact 저장 | `src/isic2024_benchmark/run_experiment.py`, `run_tabular_baselines.py`, `run_all_models.py` |
| execution export (`.py`) | leaderboard / HTML / MLflow export | `src/isic2024_benchmark/mlflow_report.py`, `mlflow_html_report.py`, `runtime_env.py` |
| runbook (`.ipynb`) | `.env` 로드, checkpoint readiness, GPU preflight, 실행 명령 정리 | `src/eda/isic2024_followup_validation_17.ipynb` |
| tabular analysis (`.ipynb`) | tabular leaderboard 비교, feature set별 `pauc_above_tpr80` 요약 | `src/eda/isic2024_followup_tabular_analysis_17.ipynb` |
| image analysis (`.ipynb`) | image readiness board, model별 best `pauc_above_tpr80` 비교 | `src/eda/isic2024_followup_image_analysis_17.ipynb` |
...
- notebook이 결과 표나 그림을 만들면 `artifacts/eda/isic2024/followup_tables` 같은 산출 경로에 저장한다.
- 최종 수치의 source of truth는 항상 `.py` runner가 남긴 `artifacts/`와 `mlruns/`이다.

## 5. GPU 사용 전략

- 기본 전략은 "한 모델을 여러 GPU로 쪼개는 것"보다 "여러 모델 또는 trial을 GPU 0/1에 병렬 배치하는 것"이다.
- 실제 GPU scheduling은 `src/isic2024_benchmark/run_all_models.py`가 담당한다.
...
3. 결과는 `artifacts/`와 `mlruns/`에 저장한다.
