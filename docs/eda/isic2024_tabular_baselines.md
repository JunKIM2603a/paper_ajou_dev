...
## Tabular baseline trial 요약

[Feature engineering / 입력]

- 입력 feature set: `strict_main_input`
- 사용 feature 수: 39개
- 입력 성격: ISIC2024 ordinary metadata만 사용
...
| 모델 | baseline 역할 | 현재 backend | 주요 설정 | 불균형 처리 |
|---|---|---|---|---|
...
| `mlp` | 기본 nonlinear neural tabular baseline | sklearn CPU 또는 torch GPU path | hidden layer `(64, 32)`, `alpha=1e-4`, `max_iter=50` | GPU torch: BCE `pos_weight`; CPU sklearn: class weight 없음 |
| `xgboost` | sparse feature에 강한 확장성 있는 GBDT baseline | XGBoost | `n_estimators=200`, `max_depth=6`, `learning_rate=0.05`, `subsample=0.8`, `colsample_bytree=0.8` | `scale_pos_weight` |
| `lightgbm` | 독립적인 고효율 GBDT 견고성 baseline | LightGBM CPU | `n_estimators=300`, `num_leaves=31`, `learning_rate=0.05`, `subsample=0.8`, `colsample_bytree=0.8` | `scale_pos_weight` |
| `catboost` | categorical feature에 강한 GBDT baseline | CatBoost | `iterations=300`, `depth=6`, `learning_rate=0.05` | `auto_class_weights="Balanced"` |
...
| `ft_transformer_external` | 외부 구현 호환성 확인 후보 | optional `rtdl_revisiting_models` | `ft_transformer`와 같은 고수준 역할 | BCE `pos_weight` |
...
- `TabNet`은 현재 tabular baseline runner가 지원하는 모델이 아니다.
...
현재 tabular runner가 인식하는 지원 모델은 다음 8개다.
...
`ft_transformer`는 repo-native PyTorch 구현이다. `ft_transformer_external`은 optional `rtdl_revisiting_models` dependency가 필요하며, package가 없으면 해당 모델만 명확한 ImportError로 실패한다.

현재 suite config인 `experiments/configs/suites/tabular_baselines.json`은 강한 tabular 비교를 위해 아래 subset을 실행 대상으로 둔다.
...
즉, 지원 모델 전체와 현재 suite 모델 subset은 의도적으로 다르다. 단일 runner나 all-model runner에서는 `logistic_regression`, `svm`, `mlp`도 명시적으로 실행할 수 있다.
