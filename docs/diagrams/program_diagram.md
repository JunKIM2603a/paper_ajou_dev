...
```mermaid
flowchart TB
    A[experiments/configs/image_baselines/모델명/config.json] --> D[run_experiment.py]
    B[src/isic2024_multimodal/run_all_models.py] --> D
    D --> E[data.py]
    E --> E1[manifest build]
    E1 --> E2[group-aware train/val/test split]
    D --> F[models.py]
    D --> G[trainer.py]
    G --> H[experiments/outputs/모델명/trial명]
    D --> I[experiments/logs/mlruns]
    D --> J[smoke options]
```
