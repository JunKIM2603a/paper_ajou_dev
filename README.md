...
이 저장소는 ISIC2024 기반 초희귀 malignant target 멀티모달 학습 연구 코드를 정리하기 위한 작업 공간이다. 현재 확정된 연구 방향은 image + tabular modeling이다. LUPI / privileged supervision 아이디어는 논문 방향이 명확히 확정되기 전까지 메인 프로젝트가 아니라 후보 연구로만 다룬다.
...
isic2024_multimodal/
  ...
  features/         # strict feature set과 train-only preprocessing spec
  ...
  research/         # 향후 LUPI 변형 등 후보 연구 아이디어
