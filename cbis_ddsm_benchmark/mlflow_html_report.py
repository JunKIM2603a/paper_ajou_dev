...
def build_html(*, experiment_name: str, tracking_uri: str, experiment_id: str, parent_runs, child_runs) -> str:
    parent_records = [row for _, row in parent_runs.iterrows()]
    child_records = [row for _, row in child_runs.iterrows()]
    generated_at = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    model_names = [safe_value(row.get("tags.model_name")) for row in parent_records]
    ...
    for parent in parent_records:
        ...
        matching_children = [
            ...
            if safe_value(row.get("tags.model_name")) == model_name
        ]
        sections.append(
            """
            <section class="panel">
            ...
            """.format(
                ...
                parent_summary=build_parent_summary(parent),
                child_table=build_child_table(matching_children, best_child_name),
            )
        )
  ...
  <div class="wrap">
    ...
    <section class="panel">
      <div class="section-head">
        <div>
          <h2>Leaderboard</h2>
          <p>Best parent runs sorted by <code>best_auc_roc</code>.</p>
        </div>
      </div>
      {leaderboard}
    </section>
...
""".format(
        ...
        leaderboard=build_leaderboard(parent_records),
        sections="\n".join(sections) if sections else '<p class="empty">No runs found.</p>',
    )


def build_leaderboard(parent_records: list) -> str:
    ...
    for row in parent_records:
        rows.append(
            ...
            f"<td>{escape(safe_value(row.get('params.best_hp_learning_rate')))}</td>"
            f"<td>{escape(safe_value(row.get('params.best_hp_weight_decay')))}</td>"
            f"<td>{escape(safe_value(row.get('params.best_hp_epochs')))}</td>"
    ...
    return (
        ...
        "<th>LR</th><th>Weight Decay</th><th>Epochs</th><th>Best Child</th>"
        "</tr></thead><tbody>"
        + "".join(rows)
        + "</tbody></table>"
    )
...
def build_parent_summary(parent) -> str:
    items = [
        ("Accuracy", format_metric(parent.get("metrics.best_accuracy"))),
        ("Precision", format_metric(parent.get("metrics.best_precision"))),
        ("Recall", format_metric(parent.get("metrics.best_recall"))),
        ("F1 Score", format_metric(parent.get("metrics.best_f1_score"))),
        ("AUC ROC", format_metric(parent.get("metrics.best_auc_roc"))),
        ("Best LR", safe_value(parent.get("params.best_hp_learning_rate"))),
        ("Best Weight Decay", safe_value(parent.get("params.best_hp_weight_decay"))),
        ("Best Epochs", safe_value(parent.get("params.best_hp_epochs"))),
    ]
    return (
...
def build_child_table(child_rows: list, best_child_name: str) -> str:
    if not child_rows:
    ...
    for row in child_rows:
        ...
        params = {
            "learning_rate": safe_value(row.get("params.hp_learning_rate")),
            "weight_decay": safe_value(row.get("params.hp_weight_decay")),
            "epochs": safe_value(row.get("params.hp_epochs")),
            "optimizer": safe_value(row.get("params.hp_optimizer")),
        }
        rows.append(
            f"<tr{row_class}>"
    ...
    return (
        "<table><thead><tr>"
        "<th>Trial Run</th><th>Hyperparameters</th><th>Accuracy</th><th>Precision</th>"
        "<th>Recall</th><th>F1</th><th>AUC</th><th>Duration (s)</th>"
        "</tr></thead><tbody>"
        + "".join(rows)
        + "</tbody></table>"
    )
